...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("KRA Branch List Payload:", payload)

    // Call KRA API with dynamic headers
    const kraRes = await fetch('https://etims-api-sbx.kra.go.ke/etims-api/selectBhfList', {
      method: 'POST',
    ...
    console.log("KRA Branch List Response:", kraData)

    return NextResponse.json({
      resultCd: kraData.resultCd,
      resultMsg: kraData.resultMsg,
      data: {
        bhfList: kraData.data?.bhfList || []
      }
    })

  } catch (error: any) {
    console.error('KRA Branch List Error:', error)
    return NextResponse.json({ 
