...
import { useSynchronizedPOSStore } from "@/stores/synchronized-pos-store"
...
export function SynchronizedPOSSystem() {
  const { toast } = useToast()
  const { availableMenuItems } = useSynchronizedPOSStore()
  const { calculateAvailablePortions } = useSynchronizedRecipesStore()
  ...
  useEffect(() => {
    ...
    window.addEventListener("recipes-updated", handleMenuUpdate)
    return () => window.removeEventListener("recipes-updated", handleMenuUpdate)
  }, [])

  const categories = ["All", ...Array.from(new Set(availableMenuItems.map((item) => item.category)))]

  const filteredRecipes = availableMenuItems.filter((recipe) => {
  ...
  const completePayment = () => {
    ...
    cart.forEach((item) => {
      const recipe = availableMenuItems.find((r) => r.id === item.originalId)
      if (recipe) {
        // Deduct ingredients from inventory
        recipe.ingredients.forEach((ingredient: any) => {
          const quantityNeeded = ingredient.quantity_per_batch * item.quantity
          updateStock(ingredient.ingredient_id, quantityNeeded, "subtract", "pos-order")
        })
      }
    })
    ...
    toast({
      ...
      description: `Order has been processed and inventory updated automatically.`,
    })
      ...
      {menuUpdateNotification && (
        <div className="bg-green-100 dark:bg-green-900 border-green-200 dark:border-green-800 border-b p-2">
          <div className="flex items-center justify-center gap-2 text-green-800 dark:text-green-200">
            ...
            <span className="text-sm font-medium">Menu updated! New recipes are now available.</span>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-4">
            ...
            <Badge variant="secondary" className="text-xs">
              {availableMenuItems.length} items available
            </Badge>
          </div>
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Badge
                              variant={
                                availablePortions > 10 ? "secondary" : availablePortions > 0 ? "outline" : "destructive"
                              }
                              className="text-xs"
                            >
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-4 text-xs text-muted-foreground">
                              ...
                              <span>Stock: {availablePortions}</span>
                              {availablePortions > 0 ? (
                                <Badge variant="secondary" className="text-xs">
                                  <CheckCircle className="h-3 w-3 mr-1" />
                                  Available
          ...
          <div className="text-center space-y-4">
            ...
            <div className="bg-blue-50 dark:bg-blue-950 p-3 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-medium text-blue-800 dark:text-blue-200">Inventory Updated</span>
              </div>
